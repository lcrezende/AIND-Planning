...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for aNode in self.a_levels[level-1]:
            for eNode in aNode.effnodes:
                aNode.children.add(eNode)
                eNode.parents.add(aNode)
                self.s_levels[level].add(eNode)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e1 in node_a1.effnodes:
            if e1 in node_a2.prenodes:
                return True
        for e2 in node_a2.effnodes:
            if e2 in node_a1.prenodes:
                return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            found_goal = False
            level_cost = 0
            # while not found_goal and not (level_cost == (len(self.s_levels) - 1)):
            while not found_goal:
                if level_cost == (len(self.s_levels) - 1):
                    # print('goal not found - #levels: ', len(self.s_levels))
                    # print('level_sum: ', level_sum, '| goal: ', g)
                    break
                for node in self.s_levels[level_cost]:
                    # print('| level_cost: ', level_cost)
                    # print('node: ', node.symbol, node.is_pos)
                    if node.is_pos and g == node.symbol:
